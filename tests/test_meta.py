...
def test_many_to_many(db: DataBase):
    class ValueModel(db.Model):
        __tablename__ = "value_model"
        id = db.IntField(primary_key=True)
        name = db.StrField(nullable=False)
    ...
    foo_1 = FooModel(values=[value_1])
    foo_2 = FooModel(values=[value_1, value_2])

    db.session.add(foo_1)
    db.session.add(foo_2)
    db.session.commit()

    assert foo_1.values == [value_1]
    assert foo_2.values == [value_1, value_2]
